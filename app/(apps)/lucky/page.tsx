  ...
  useEffect(() => {
    ...
    const initializeLottery = async () => {
      try {
        ...
        // Get current round
        const roundResponse = await fetch('/api/lottery?action=current-round');
  ...
  useEffect(() => {
    const fetchUserTickets = async () => {
      if (walletAddress && currentRoundId) {
        try {
          const ticketsResponse = await fetch(`/api/lottery?action=user-tickets&wallet=${walletAddress}&roundId=${currentRoundId}`);
  ...
  const buyTicket = useCallback(async () => {
    ...
    try {
      ...
      if (data.success) {
        ...
        if (walletAddress && currentRoundId) {
          try {
            const finalTicketsResponse = await fetch(`/api/lottery?action=user-tickets&wallet=${walletAddress}&roundId=${currentRoundId}`);
