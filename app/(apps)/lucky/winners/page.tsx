  ...
  useEffect(() => {
    const fetchAllWinners = async () => {
      try {
        ...
        const response = await fetch('/api/lottery?action=recent-winners&limit=1000');
      ...
      <header className="px-3 sm:px-4 py-4 sm:py-6">
        <nav className="max-w-7xl mx-auto flex items-center justify-between gap-2">
          <div className="flex items-center space-x-2 sm:space-x-3 min-w-0 flex-shrink">
            ...
            <Link href="/" className="text-lg sm:text-2xl font-bold bg-gradient-to-r from-pink-400 to-blue-400 bg-clip-text text-transparent hover:opacity-80 transition-opacity truncate">
              LuckyHaus v3.1.1
            </Link>
          </div>
