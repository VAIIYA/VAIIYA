...
export default async function MemeHausPage() {
    const t = await getTranslations('MemeHausPage');
        ...
        <main className="min-h-screen bg-white">
                            ...
                            <div className="relative border-4 border-metamask-purple/5 bg-white p-2 rounded-[40px] shadow-2xl overflow-hidden shadow-orange-500/10">
                                <div className="bg-metamask-purple text-white p-8 rounded-[32px]">
                                    <div className="flex justify-between items-center mb-8">
                                        <div className="font-serif text-2xl">{t('dashboardTitle')}</div>
                                        <div className="text-xs font-mono bg-white/10 px-2 py-1 rounded">CONNECTED_WALLET: 7vhY...9Pq1</div>
                                    </div>
                                    <div className="space-y-4">
                                        <div className="bg-white/5 p-4 rounded-2xl border border-white/10">
                                            <div className="text-gray-400 text-xs mb-1">{t('kickbacksTotal')}</div>
                                            <div className="text-2xl font-serif text-solana-green">24.5 SOL</div>
                                        </div>
                                        <div className="grid grid-cols-2 gap-4">
                                            <div className="bg-white/5 p-4 rounded-2xl border border-white/10">
                                                <div className="text-gray-400 text-xs mb-1">{t('tokensCreated')}</div>
                                                <div className="text-xl font-serif">12</div>
                                            </div>
                                            <div className="bg-white/5 p-4 rounded-2xl border border-white/10">
                                                <div className="text-gray-400 text-xs mb-1">{t('volGenerated')}</div>
            ...
            <section className="py-24 px-4 sm:px-6 lg:px-8">
                <div className="max-w-4xl mx-auto text-center">
                    <h2 className="text-4xl font-serif mb-6 text-metamask-purple">{t('kickbackTitle')}</h2>
                    <p className="text-xl text-gray-600 mb-12 leading-relaxed">
                        {t('kickbackDesc')}
                    </p>
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
                        <div className="card-vibe border-metamask-orange/20">
                            <div className="text-3xl font-serif text-metamask-orange mb-2">{t('fairLaunchTitle')}</div>
                            <p className="text-sm text-gray-500">{t('fairLaunchDesc')}</p>
                        </div>
                        <div className="card-vibe border-purple-500/20">
                            <div className="text-3xl font-serif text-metamask-purple mb-2">{t('instantMintTitle')}</div>
                            <p className="text-sm text-gray-500">{t('instantMintDesc')}</p>
                        </div>
                        <div className="card-vibe border-solana-green/20">
                            <div className="text-3xl font-serif text-solana-green mb-2">{t('autoLpTitle')}</div>
                            <p className="text-sm text-gray-500">{t('autoLpDesc')}</p>
                        </div>
                    </div>
                </div>
            </section>
            ...
            <section className="py-24 bg-metamask-purple text-white">
                <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
                    <h2 className="text-5xl font-serif italic mb-8">{t('quote')}</h2>
